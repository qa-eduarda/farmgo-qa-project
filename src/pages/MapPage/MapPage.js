...
function MapPage() {
  ...
  useEffect(() => {
    ...
    function generateMarkersPosition(latitude, longitude) {
      return [
        { lat: latitude + 0.0018, lng: longitude + 0.0012, title: "Parceiro Orgânico Sol Nascente" },
        { lat: latitude - 0.0012, lng: longitude + 0.0025, title: "Sítio Alvorada de Frutas" },
        { lat: latitude + 0.0028, lng: longitude - 0.0015, title: "Chácara Verde Vida (Hortaliças)" },
        { lat: latitude - 0.0025, lng: longitude - 0.0022, title: "Fazenda Nova Esperança (Laticínios)" }
      ];
    }
    ...
    const initLeafletMap = (lat, lng) => {
      ...
      positions.forEach((pos) => {
        window.L.marker([pos.lat, pos.lng]).addTo(map)
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        console.error("Geolocalização indisponível. Usando ponto padrão (Av. Paulista, SP): ", error);
        const defaultLat = -23.5617;
        const defaultLng = -46.6560;
            ...
            <div className="col-lg-10">
              <div className="text-center mb-4">
                <p className="text-muted leading-relaxed" style={{ fontSize: '1.1rem' }}>
                  Veja no mapa interativo abaixo os produtores familiares e parceiros mais próximos da sua residência. 
                  Aceite a permissão de geolocalização para resultados baseados no seu endereço.
                </p>
              </div>
                ...
                >
                  <div className="d-flex align-items-center justify-content-center h-100 bg-light text-muted">
                    <div className="text-center">
                      <div className="spinner-border spinner-border-sm mb-2 text-success" role="status"></div>
